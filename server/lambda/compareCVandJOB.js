...
connect("mongodb://127.0.0.1:27017/hire_it", { useNewUrlParser: true, useUnifiedTopology: true }).then()

exports.handler = async (event) => {
  var { jobId, applicationId } = event.queryStringParameters
  
  if(jobId && applicationId){
    var jobObj = await JobPost.findById(jobId)
    var applicationCV = await jobCV.findById(applicationId);
    ...
    await profileMatcher.updateOne({ "jobId": jobObj._id, "applicationId": applicationId }, { $set: matcherObject }, { upsert: true, new: true });
    const response = {
      statusCode: 200, 
      body: "success"
    }
